...
# 개인정보를 직접 요구하는 질의
# 🔴 `주소`는 사람에 붙을 때만 PII다. 법인 주소(본점 소재지)는 공시의 정상 항목이라
#    전역으로 잡으면 답변 가능한 질의를 거절한다 — 정확성·요구사항 충족 감점
#    (2026-09-03 실측: "회사의 주소지는 어디인가" → 개인정보 거절).
PII_REQUEST = re.compile(
    r"생년월일|생일|나이|출생|주민\s*등록|주민\s*번호|성별|"
    r"개인\s*정보|연락처|이메일|전화\s*번호|"
    r"(임원|대표|사장|이사|직원|개인)[^.\n]{0,10}(개인|신상|프로필|인적\s*사항|주소)"
)
