...
INTENT_PATHS: tuple[tuple[str, str, tuple[str, ...]], ...] = (
    # 🔴 조사(의/에/을)를 허용한다. `사업\s*개요`는 "사업**의** 개요"에 막혔다 —
    #    골드셋 실패 1건의 원인이었고, 규칙이 조사 하나에 뚫리는 건 구조적 결함이다.
    ("core_business",
     r"핵심\s*사업|주요\s*사업|사업\s*(?:의\s*)?개요|사업\s*(?:의\s*)?내용|어떤\s*사업",
     ("II-1", "II-2")),
    ...
    # 🔴 배당 규칙이 **아예 없어서** 골드셋 실패 3건이 났다 (DB 실측: III-6 = 배당에
    #    관한 사항, 900건). LLM 라우팅으로도 잡히지만 그건 429에 취약하다 —
    #    부하 실측에서 재시도 소진 52건이 나왔고, 그때 라우팅이 죽으면 검색으로
    #    떨어져 이 실패가 되살아난다. **아는 것은 규칙으로 내린다.**
    ("dividend", r"배당|주주\s*환원|현금배당|배당성향|배당금\s*총액", ("III-6",)),
