...
def rrf_fuse(rankings: list[list[SearchHit]], *, k: int = 60,
    ...
    본 코퍼스에서는 **모든 가중치 조합에서 최악이었다**. 55문항 실측 (2026-08-25):
    골든셋 25문항(파라미터 튜닝에 사용) + 홀드아웃 30문항(튜닝 미사용 기업).

        구성            골든셋   홀드아웃   ← 판단 기준은 홀드아웃
        BM25 단독       0.290    0.117
        벡터 단독       0.502    0.429
        RRF k=60 w=1    0.419    0.258     ← 표준값. BM25 단독보다야 낫지만 최하위
        RRF k=10 w=2    0.620    0.413     ← 골든셋 최고. 홀드아웃에선 벡터에 진다
        RRF k=5  w=3    0.538    0.436     ← 채택 (홀드아웃 최고 · hit@8도 최고)

    k가 크면 상위 랭크의 우위가 평탄해진다. 두 팔의 품질이 비슷할 때는 그게
    안정적이지만, 여기처럼 한쪽(벡터)이 확연히 우수하면 **좋은 팔을 나쁜 팔이
    끌어내린다**. k를 줄이고 우수한 팔에 가중치를 주면 그 희석이 사라진다.

    🔴 **골든셋 최고값을 쓰지 않은 이유**: k=10/w=2는 그 25문항으로 고른 값이라
    낙관 편향이 있다. 실제로 홀드아웃에서 우위가 사라졌다(0.413 < 벡터 0.429).
    ⚠️ 다만 k=5/w=3도 홀드아웃을 **보고** 골랐으므로 이제 그 세트도 오염됐다.
    홀드아웃 기준 상위 조합들의 차이(0.41~0.44)는 30문항에서 노이즈 범위다 —
    **견고한 결론은 "k=60을 쓰지 마라"이지 특정 조합의 우월성이 아니다.**

    🔴 **그럼에도 하이브리드를 유지하는 이유는 커버리지다.** 벡터 스토어는
    최신 사업보고서만 담는다(분기·반기·비정기 공시 제외). 벡터 단독으로 가면
    커버리지 밖 질의에서 검색이 죽지만, 융합은 그 구간을 BM25가 받는다.
    """
