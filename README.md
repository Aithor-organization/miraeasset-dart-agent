...
### 릴리즈 게이트 — AITHOR `eval-audit` 4축 + 회귀 (2026-08-24 실측)

```
quality 186/186 · safety 0건 · regression 0건 · p95 27.4s · cost 1,195 토큰/문항
```
...
| latency | p95 27.4s · max 47.2s | < 240s | 평가 타임아웃(300초) 초과 |
| **cost** | 1,195 토큰/문항 | < 4,000 | 크레딧 소진 |
...
### 하이브리드 검색 — A/B 실측 (2026-08-24)

골든셋 `section` 25문항으로 검색 계층만 직접 측정했다 (`eval/retrieval_ab.py`).
서버 E2E가 아니라 **retrieval 레벨**인 이유는, section 질문이 운영 경로에서
`get_section`(결정론 주소 조회)으로 답해 `doc_search`의 개선이 관측되지 않기 때문이다.
`doc_search`는 그 주소 추론이 실패했을 때의 폴백이며, **미지의 평가 질문에서는 이 폴백이
곧 답변 품질**이다.

| 검색 팔 | hit@1 | hit@3 | hit@8 | MRR |
|---|---:|---:|---:|---:|
| BM25 단독 (기존 운영값) | 6/25 | 8/25 | 9/25 | 0.290 |
| 벡터 단독 (bge-m3) | 9/25 | 16/25 | 18/25 | 0.502 |
| RRF k=60, 1:1 (표준값) | 8/25 | 10/25 | 17/25 | 0.419 |
| **RRF k=10, 벡터 가중 2배 (채택)** | **14/25** | **16/25** | **18/25** | **0.620** |

🔴 **표준 RRF(k=60)가 벡터 단독보다 나빴다.** k가 크면 상위 랭크의 우위가 평탄해져,
확연히 우수한 팔을 열등한 팔이 끌어내린다. k를 줄이고 가중치를 주자 희석이 사라졌다
(`HYBRID_RRF_K=10` · `HYBRID_VEC_WEIGHT=2.0`, `retrieval/bm25.py:rrf_fuse` docstring).

⚠️ **한계**: 25문항 튜닝이라 과적합 여지가 있고, 홀드아웃 검증을 하지 않았다.
벡터 커버리지도 부분이다 — 임베딩된 섹션만 벡터 팔에 뜨고 나머지는 BM25로만 랭킹된다.

**운영 기본은 여전히 OFF** (`DART_HYBRID=1`일 때만 발동). 벡터 스토어나 CLOVA 키가
없으면 자동으로 BM25 단독으로 강등되므로, 하이브리드가 죽어도 검색은 살아 있다.
